...
const AddOnPage = () => {
  ...
  useEffect(() => {
    const fetchAddOns = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/addons'); // Update with your API URL
      ...
      <div style={styles.grid}>
        {addOns.map((addon) => (
          <div key={addon._id} style={styles.card}>
            <img
              src={`http://localhost:5000/${addon.gambar}`} // Ensure the image URL is correct
              alt={addon.nama}
              style={styles.image}
            />
